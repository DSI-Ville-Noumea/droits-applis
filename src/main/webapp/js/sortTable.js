...
function sortTable(td_element,ignoreFirstLines,ignoreLastLines) {
 ...
 ignoreFirstLines = (typeof(ignoreFirstLines)=='undefined') ? 0 : ignoreFirstLines;

 var sortImages =['images/desc.gif','images/asc.gif','images/none.gif','images/sorting.gif']; 
 ...
 for (var i=ignoreFirstLines; i<tr_elements.length - ignoreLastLines; i++) {
...
for (var i=0; i<ignoreFirstLines; i++) { 
 	originalCol.shift();
 	}
 ...
 for (var i=ignoreFirstLines; i < tr_elements.length - ignoreLastLines; i++) { 
  var old_row = sortCol[i-ignoreFirstLines][1];
  var new_row = i;
  ...
  makePretty(table_element,indexCol,ignoreFirstLines, ignoreLastLines);
...
function makePretty(tblEl, col, ignoreFirstLines, ignoreLastLines) {

  var i, j;
  var rowEl, cellEl;

  // Set style classes on each row to alternate their appearance.
  for (i = ignoreFirstLines; i < tblEl.rows.length - ignoreLastLines; i++) {
   rowEl = tblEl.rows[i];
